from PySide6.QtWidgets import QMainWindow, QTextEdit, QLineEdit, QLabel, QGridLayout, QWidget, QVBoxLayout, QHBoxLayout, QSplitter
...
class EditorWindowUI(QWidget):  # Changed from QMainWindow to QWidget
    ...
    def _buildUI(self):
        ...
        for i in range(6):
            ...
            input_field.textChanged.connect(lambda text, idx=i: self.testInputChanged.emit(idx+1, text))
            expected_field.textChanged.connect(lambda text, idx=i: self.testOutputChanged.emit(idx+1, text))
            
        self.mainLayout.addLayout(self.testsGrid)
        
        # Connect definition text change
        self.schemeDefinitionView.textChanged.connect(
            lambda: self.definitionTextChanged.emit(self.schemeDefinitionView.toPlainText())
        )
