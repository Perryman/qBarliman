...
from qBarliman.constants import LOAD_MK_VICARE_SCM, LOAD_MK_SCM, TEST_TIMEOUT_MS
...
class QueryBuilder:
    ...
    def build_simple_query(self, scheme_document):
        querySimple = self._make_query_string(
            defns=scheme_document.get_definitionText,
            body=",_",
            expectedOut="q",
            simple=True,
            name="-simple",
    ...
    def build_all_tests_query(self, scheme_document):
        allTestInputs = " ".join(scheme_document.get_testInputs)
        allTestOutputs = " ".join(scheme_document.get_testExpected)
        return ALL_TEST_WRITE_T.substitute(
            definitionText=scheme_document.get_definitionText,
